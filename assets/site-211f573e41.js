...
class DewpointForecast extends React.Component {
  ...
  componentDidMount() {
    ...
    this.getUserLocation(); // Connect the initLookup function within this class to the global window context, so Google Maps can invoke it
  ...
  getDiscomfortLevel(dewpoint) {
    let roundedDewpoint = Math.round(dewpoint),
        levelText = '',
        dpClass = '',
        levelIsFound = false,
        thisLevel = null,
    ...
    scale.forEach(function (value, i) {
      if (!levelIsFound) {
        if (Cookies.get('units') != null && Cookies.get('units') == 'si' && roundedDewpoint < value['c'] || (Cookies.get('units') != null && Cookies.get('units') == 'us' || Cookies.get('units') == null) && roundedDewpoint < value['f']) {
          levelIsFound = true;
          thisLevel = value;
        }
      }
  ...
  render() {
    ...
    let dailyData = this.state.weather != null ? this.state.weather.daily.data.slice(1).map(day => React.createElement("div", {
      ...
      className: "date"
    }, this.formatDate(day.time, this.state.weather.offset)), React.createElement("img", {
      className: "dewdrop-icon",
      src: "/image/drop-silhouette.svg"
